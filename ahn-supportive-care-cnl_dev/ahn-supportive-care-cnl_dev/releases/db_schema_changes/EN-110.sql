...
BEGIN
	...
	VALUES
		...
		(IFNULL(@current_authenticated_user_id, 0), NEW.id, 'master_users', 'customer_number', NEW.customer_number),
...
BEGIN
	...
	IF NEW.customer_number != OLD.customer_number
	THEN
		INSERT INTO master_audit (admin_id, table_id, table_name, column_name, old_value, new_value) VALUES(IFNULL(@current_authenticated_user_id, 0), NEW.id, 'master_users', 'customer_number', OLD.customer_number, NEW.customer_number);
	END IF;
