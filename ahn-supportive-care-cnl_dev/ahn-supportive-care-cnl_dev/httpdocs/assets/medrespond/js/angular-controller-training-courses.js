function trainingCoursesController($scope, $rootScope, $http, $injector, $q, $sce, $cookieStore, $location)
{
		...
		$scope.can_show_take_course_button = function(this_all_courses_index) {
			// if this course is active then show the take course button
			if($scope.all_courses[this_all_courses_index].is_active === '1') {
				return true;
			}

			// uncomment the return false statement below to prevent the user from being able to take other courses after finishing their first
			//return false;

			// if the course has been completed and passed then don't show the button
			if($scope.all_courses[this_all_courses_index].has_completed  === '1' && $scope.all_courses[this_all_courses_index].has_passed === '1') {
				return false;
			}

			// if there are any other active courses don't show the take course button for this course
			for(i = 0; i < $scope.all_courses.length; i++) {
				if(i !== this_all_courses_index) {
					if(typeof $scope.all_courses[i] !== 'undefined') {
						if($scope.all_courses[i].is_active === '1') {
							return false;
						}
					}
				}
