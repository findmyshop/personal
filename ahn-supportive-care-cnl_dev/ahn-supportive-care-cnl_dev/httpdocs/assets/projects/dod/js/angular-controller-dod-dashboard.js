function trainingDashboardController($scope, $rootScope, $http, $q, $timeout /*, existingUserService */)
{
	...
	$scope.edit_user_select_country_changed = function(country_id)
	{
		//console.log('country_id: ' + country_id);
		...
		{
			$scope.us_address_on_edit_form = false;
		}
	}
	...
	$scope.init_existing_user = function(user)
	{
		$http.post('/admin/ajax_angular_get_user', {'user_id': $scope.user.id}).
				success(function(data, status, headers, config)
				{
					...
					$scope.edit_user_select_type_changed(data.user.user_type_id);
				}).
				error(function(data, status, headers, config)
				{
					alert('get_users error');
		...
		$timeout(function() {
			$scope.$parent.$apply(function () {
					$scope.new_user = {
						...
						middle_initial: '',
						last_name:		'',
						department_id:	-1, /*$scope.departments[0].department_name,*/
						...
						province:			'',
						zip:				'',
						country_id:		-1, /*$scope.countries[0].code,*/
						login_enabled:		1
					};
