...
//echo $this->db->last_query();die;
	...
	public function get_course_content_knowledge_test_id($course_id)
	{
		log_info(__FILE__, __LINE__, __METHOD__, "course_id = $course_id");
		...
		$query = $this->db
			...
			->where(array(
					'course_id'	=> $course_id,
					'test_name'	=> 'Content/Knowledge Questions'
				))
			->limit(1)
			->get();
	...
	public function get_content_knowlege_test_stats($user_id, $course_id, $course_iteration, $only_active_courses = TRUE)
	{
		...
		$test_id = $this->get_course_content_knowledge_test_id($course_id);
	...
	public function get_courses() {
		...
		$query = $this->db->where(array('organization_id' => PROPERTY_ORGANIZATION_ID, 'active' => 1))->get('master_courses');
	...
	public function get_course_price($course_name)
	{
		log_info(__FILE__, __LINE__, __METHOD__, "Called with course_name = $course_name");

		$query = $this->db
			->select('price')
			->from('master_courses')
			->where('course_name', $course_name)
			->where('organization_id', PROPERTY_ORGANIZATION_ID)
			->get();
	...
	public function get_raw_courses_dump($search = NULL, $course_id = NULL, $role_id = NULL, $limit = NULL, $offset = NULL, $return_query_object = TRUE) {
		log_info(__FILE__, __LINE__, __METHOD__, 'Called');
		...
		if($course_id !== NULL && is_numeric($course_id)) {
			$course_id_where = " AND master_courses.id = $course_id";
		}
	...
	public function get_test_elements_by_response($response_id)
	{
	...
	public function get_user_answers($search = NULL, $course_id = NULL, $role_id = NULL, $limit = NULL, $offset = NULL, $return_query_object = FALSE)
	{
		log_info(__FILE__, __LINE__, __METHOD__, 'Called');
	...
	public function get_user_courses($user_id = FALSE, $all = TRUE, $search = FALSE, $uncompleted = NULL, $not_passed = NULL, $no_cert = NULL, $course_id = NULL, $role_id = NULL, $limit = NULL, $offset = NULL, $return_query_object = FALSE) {
		log_info(__FILE__, __LINE__, __METHOD__, 'Called');
		...
		$role_id_where = "";
			...
			else
			{
				...
				$stmt = "
					...
					ORDER BY master_users.last_name, master_users.first_name, master_users.middle_initial, master_courses.course_name, master_courses.id, master_user_courses.current_iteration
			";
		}
	...
	public function update_master_user_courses_total_sections_visited($user_id, $course_id, $course_iteration)
	{
		...
		$sql = "
			...
			SET master_user_courses.total_sections_visited = (
				SELECT COUNT(DISTINCT master_user_course_activity.response_id)
				...
				WHERE
						master_course_elements.course_id = $course_id
